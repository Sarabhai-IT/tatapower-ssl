...
GROUP BY 
    ...
    B.BUNAME,
    B.BUCODE,
    D.DSRDATE  -- Group by all columns except the ones being aggregated
...
async function insertOrUpdateDashboard(incidentData, date) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_DAILY_SECAUTO]`;
            ...
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_DAILY_SECAUTO]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_DAILY_SECAUTO]
                    SET 
                        [AVAILABLE] = @AVAILABLE,
                        [WORKING] = @WORKING,
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_DAILY_SECAUTO]
                    ([ROWID],  [VID], [BUID], [SIID],[VNAME], [BUNAME], [SINAME], [VCODE], [BUCODE], [SICODE], [DATE], 
                     [AVAILABLE], [WORKING] , [NOTWORKING] , 
