...
SELECT 
    ...
    -- Calculate the average per day by dividing the sum by the distinct days
	    (SUM(COALESCE(D.AVAILABLE, 0)) / COUNT(DISTINCT D.DSRDATE)) AS AVAILABLE_MONTHLY,
    (SUM(COALESCE(D.WORKING, 0)) / COUNT(DISTINCT D.DSRDATE)) AS WORKING_MONTHLY,

    (SUM(COALESCE(D.NOTWORKING, 0)) / COUNT(DISTINCT D.DSRDATE)) AS NOTWORKING_MONTHLY,
...
AND S.SISTATUS = 'ACTIVE'  -- Only active sites
    ...
    AND COALESCE(D.AVAILABLE, 0) != 0  -- Exclude rows where AVAILABLE is 0 or NULL
GROUP BY 
    S.VID,
    S.BUID,
    S.SIID,
...
async function insertOrUpdateDashboard(incidentData) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const required = incidentRow.WORKING_MONTHLY;  // Availability data
            const gap = incidentRow.NOTWORKING_MONTHLY;  // Availability data
 
            // Fetch the current max ROWID for each iteration and increment it by 1
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO]`;
            ...
            // Check if the record exists in OL_DASHBOARD_MONTHLY_SECAUTO
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO]
                    SET
                        [AVAILABLE] = @AVAILABLE,
                        [WORKING] = @WORKING,
                ...
                await pool.request()
                    ...
                    .input('WORKING', sql.Int, required)
                    .input('NOTWORKING', sql.Int, gap)
                    .input('MONTHNAME', sql.NVarChar, monthName)
                    .input('SITECOUNT' , sql.Int , 1)
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO]
                    ([ROWID], [VID], [BUID], [SIID], [VNAME], [BUNAME], [SINAME], [VCODE], [BUCODE], [SICODE], [MONTH],
                     [AVAILABLE],[WORKING],[NOTWORKING], [QUARTER], [MONTHNAME], [YEAR], [SITECOUNT])
                    VALUES
                    (@ROWID, @VID , @BUID , @SIID , @VNAME, @BUNAME, @SINAME, @VCODE, @BUCODE, @SICODE, @MONTH,
                     @AVAILABLE, @WORKING, @NOTWORKING, @QUARTER, @MONTHNAME , @YEAR, @SITECOUNT);
                `;
