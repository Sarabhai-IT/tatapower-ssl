...
// const endDate = '2025-02-28';    // End Date
...
WHERE 
    ...
    AND (CAST(ISNULL(CASE WHEN D.CCTV = 'null' THEN 0 ELSE D.CCTV END, 0) AS INT) != 0)  -- Exclude rows where CCTV is 0
ORDER BY 
...
async function insertOrUpdateDashboard(incidentData, date) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_DAILY_CCTV]`;
            ...
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_DAILY_CCTV]
            ...
            if (recordExists) {
                ...
                const updateQuery = ` 
                    UPDATE [dbo].[OL_DASHBOARD_DAILY_CCTV]
                    SET 
                        [CCTV] = @CCTV,
                        [MONTH] = @MONTH,
                        [QUARTER] = @QUARTER,
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_DAILY_CCTV]
...
async function fetchAndInsertIncidentData() {
    try {
        ...
        // Fetch Incident Data for the given date range (D-1)
