...
FROM 
    DSRSECSTAFFONROLL D  -- Data comes from DSRSECSTAFFONROLL
...
async function insertOrUpdateDashboard(incidentData, date) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_DAILY_ONROLL]`;
            ...
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_DAILY_ONROLL]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_DAILY_ONROLL]
                    SET 
                        [AVAILABLE] = @AVAILABLE,
                        [MONTH] = @MONTH,
                        [QUARTER] = @QUARTER,
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_DAILY_ONROLL]
