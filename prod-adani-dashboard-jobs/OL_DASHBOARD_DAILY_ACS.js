...
// const startDate = '2024-08-01';  // Start Date
// const endDate = '2024-08-31';    // End Date
...
// const startDate = '2024-12-01';  // Start Date
...
WHERE 
    ...
    AND (CAST(ISNULL(CASE WHEN D.ACS = 'null' THEN 0 ELSE D.ACS END, 0) AS INT) != 0)  -- Exclude rows where ACS is 0
ORDER BY 
...
async function insertOrUpdateDashboard(incidentData, date) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_DAILY_ACS]`;
            ...
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_DAILY_ACS]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_DAILY_ACS]
                    SET 
                        [ACS] = @ACS,
                        [MONTH] = @MONTH,
                        [QUARTER] = @QUARTER,
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_DAILY_ACS]
