...
SELECT 
    ...
    (SUM(COALESCE(D.AVAILABLE, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_AVAILABLE,
	    (SUM(COALESCE(D.AUTHORIZED, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_AUTHORIZED,
    (SUM(COALESCE(D.DEFICIENCY, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_DEFICIENCY,
...
async function insertOrUpdateDashboard(incidentData) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_MONTHLY_AGENCY]`;
            ...
            // Check if the record exists in OL_DASHBOARD_MONTHLY_AGENCY
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_MONTHLY_AGENCY]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_MONTHLY_AGENCY]
                    SET
                        [AVAILABLE] = @AVAILABLE,
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_MONTHLY_AGENCY]
                    ([ROWID], [VID], [BUID], [SIID], [VNAME], [BUNAME], [SINAME], [VCODE], [BUCODE], [SICODE], [MONTH],
                     [AVAILABLE],[AUTHORIZED],[DEFICIENCY], [QUARTER], [MONTHNAME], [YEAR])
                    VALUES
                    (@ROWID, @VID , @BUID , @SIID , @VNAME, @BUNAME, @SINAME, @VCODE, @BUCODE, @SICODE, @MONTH,
                     @AVAILABLE, @AUTHORIZED, @DEFICIENCY, @QUARTER, @MONTHNAME , @YEAR);
                `;
