...
SELECT
    ...
    (SUM(ISNULL(D.WORKING, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_WORKING,
    -- Calculate the monthly average for DEFICIENCY per day
    (SUM(ISNULL(D.NOTWORKING, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_NOTWORKING,
...
async function insertOrUpdateDashboard(incidentData) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const available = incidentRow.MONTHLY_WORKING;  // Availability data
            const required = incidentRow.MONTHLY_AVAILABLE;  // Availability data
            const gap = incidentRow.MONTHLY_NOTWORKING;  // Availability data
            const dsrParamsName = incidentRow.DSRPARAMSNAME;

            // Fetch the current max ROWID for each iteration and increment it by 1
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO_PARAMS]`;
            ...
            // Check if the record exists in OL_DASHBOARD_MONTHLY_SECAUTO_PARAMS
            const checkQuery = `
                SELECT COUNT(*) AS RecordCount
                FROM [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO_PARAMS]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO_PARAMS]
                    SET
                        [WORKING] = @WORKING,
                        [AVAILABLE] = @AVAILABLE,
                ...
                await pool.request()
                    ...
                    .input('QUARTER', sql.Int, quarter)
                    .input('YEAR', sql.Int, year)
                    .input('WORKING', sql.Int, available)
                    .input('AVAILABLE', sql.Int, required)
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_MONTHLY_SECAUTO_PARAMS]
                    ([ROWID], [VID], [BUID], [SIID], [VNAME], [BUNAME], [SINAME], [VCODE], [BUCODE], [SICODE], [MONTH],
                     [WORKING], [AVAILABLE], [NOTWORKING], [QUARTER], [MONTHNAME], [YEAR], [DSRPARAMSNAME])
                    VALUES
                    (@ROWID, @VID, @BUID, @SIID, @VNAME, @BUNAME, @SINAME, @VCODE, @BUCODE, @SICODE, @MONTH,
                     @WORKING, @AVAILABLE, @NOTWORKING, @QUARTER, @MONTHNAME, @YEAR, @DSRPARAMSNAME);
                `;
