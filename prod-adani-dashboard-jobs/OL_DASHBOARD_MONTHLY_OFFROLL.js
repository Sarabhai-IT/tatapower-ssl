...
SELECT 
    ...
    (SUM(COALESCE(D.AVAILABLE, 0)) / COUNT(DISTINCT D.DSRDATE)) AS MONTHLY_AVERAGE,
...
WHERE 
    D.DSRDATE BETWEEN @startDate AND @endDate  -- Extended date range to cover the whole year
...
async function insertOrUpdateDashboard(incidentData) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const available = incidentRow.MONTHLY_AVERAGE;  // Availability data
 
            // Fetch the current max ROWID for each iteration and increment it by 1
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_MONTHLY_OFFROLL]`;
            ...
            // Check if the record exists in OL_DASHBOARD_MONTHLY_OFFROLL
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_MONTHLY_OFFROLL]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_MONTHLY_OFFROLL]
                    SET
                        [AVAILABLE] = @AVAILABLE,
                        [MONTHNAME] = @MONTHNAME
                    WHERE
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_MONTHLY_OFFROLL]
                    ([ROWID], [VID], [BUID], [SIID], [VNAME], [BUNAME], [SINAME], [VCODE], [BUCODE], [SICODE], [MONTH],
                     [AVAILABLE], [QUARTER], [MONTHNAME], [YEAR])
                    VALUES
                    (@ROWID, @VID , @BUID , @SIID , @VNAME, @BUNAME, @SINAME, @VCODE, @BUCODE, @SICODE, @MONTH,
                     @AVAILABLE, @QUARTER, @MONTHNAME , @YEAR);
                `;
