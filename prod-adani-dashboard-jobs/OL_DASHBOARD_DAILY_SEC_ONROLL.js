...
// const endDate = '2024-12-30';    // End Date
...
SELECT 
    ...
    -- Sum the REQUIRED values, exclude rows where REQUIRED is 0 or NULL
    ...
    SUM(ISNULL(D.REQUIRED, 0)) AS REQUIRED,
    
    -- Sum the GAP values, replacing NULLs with 0
    SUM(ISNULL(D.GAP, 0)) AS GAP

    
FROM 
    DSRSECSTAFFONROLL D  -- Data comes from DSRSECAUTO
...
WHERE 
    ...
    -- The condition for REQUIRED: exclude rows where REQUIRED is 0 or NULL
...
async function insertOrUpdateDashboard(incidentData, date) {
    ...
    try {
        ...
        for (const incidentRow of incidentData) {
            ...
            const maxRowIdQuery = `SELECT MAX(ROWID) AS MaxRowID FROM [dbo].[OL_DASHBOARD_DAILY_SEC_ONROLL]`;
            ...
            const checkQuery = `SELECT COUNT(*) AS RecordCount
                                FROM [dbo].[OL_DASHBOARD_DAILY_SEC_ONROLL]
            ...
            if (recordExists) {
                ...
                const updateQuery = `
                    UPDATE [dbo].[OL_DASHBOARD_DAILY_SEC_ONROLL]
                    SET 
                        [AVAILABLE] = @AVAILABLE,
                        [REQUIRED] = @REQUIRED,
                        [GAP] = @GAP,
                ...
                await pool.request()
                    ...
                    .input('REQUIRED', sql.Int, incidentRow.REQUIRED)
                    .input('GAP', sql.Int, incidentRow.GAP)
                ...
                // Insert new record
                const insertQuery = `
                    INSERT INTO [dbo].[OL_DASHBOARD_DAILY_SEC_ONROLL]
